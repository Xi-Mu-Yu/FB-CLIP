...
def image_level_metrics(results, obj, metric):
    ...
    if len(unique_labels) == 1:
        print(f"Warning: Only one class present in {obj} (class: {unique_labels[0]}). Cannot compute {metric}.")
        return 0  # 返回中性值
...
def pixel_level_metrics(results, obj, metric):
    ...
    if metric == 'pixel-auroc':
        ...
        if len(unique_labels) == 1:
            print(f"Warning: Only one class present in {obj} pixel labels (class: {unique_labels[0]}). Cannot compute {metric}.")
            return 0  # 返回中性值
        performance = roc_auc_score(gt_flat, pr_flat)
    elif metric == 'pixel-aupro':
        if len(gt.shape) == 4:
            gt = gt.squeeze(1)
        if len(pr.shape) == 4:
